...
public class ProductController {
    ...
    public String showListPaging(@RequestParam(name = "page", defaultValue = "0") int page, Model model) {
        Page<Product> productList = productService.findAllPaging(PageRequest.of(page, 2));
        model.addAttribute("products", productList);
        return "product/listPaging";
    }

    @GetMapping("/search")
    public String doSearch(Model model, Optional<String> search, Pageable pageable) {
        Page<Product> products;
        if (search.isPresent()) {
            products = productService.findByProductNameContaining̣̣̣̣̣̣(search.get(), pageable);
        } else {
